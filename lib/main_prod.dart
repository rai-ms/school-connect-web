  ...
  runZonedGuarded<Future<void>>(
    ...
    (error, stackTrace) async {
      await CrashlyticsService.service.logError(
        ...
        reason: 'Uncaught error in runZonedGuarded (prod)',
        fatal: true,
      );

      if (kDebugMode) {
        Log.crash(error: error, stackTrace: stackTrace, reason: 'main_prod');
      }
      exit(1);
    },
  );
...
FVoid appInit() async {
  ...
  try {
    // Initialize Firebase (uses google-services.json from android/app/src/prod/)
    ...
    Log.d("PROD: All Services Initialized - Base URL: ${F.baseUrl}");
  } catch (e, stack) {
    Log.e("Error in app running $e");
    await CrashlyticsService.service.logError(
      error: e,
      stackTrace: stack,
      reason: 'Error during app initialization (prod)',
      fatal: true,
    );
  }
}
